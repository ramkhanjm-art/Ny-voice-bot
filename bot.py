...
def start(m):
    ...
    bot.send_message(m.chat.id, "👋 សួស្តី! ខ្ញុំអាចបកប្រែ និងឆ្លើយសំណួរ AI បាន។", reply_markup=get_kb(m.chat.id))
...
def handle_all(m):
    ...
    if t == "👩 សំឡេងស្រី (ផ្អែម)":
        ...
        bot.send_message(cid, "✅ សំឡេងស្រីរួចរាល់", reply_markup=get_kb(cid))
    elif t == "👨 សំឡេងប្រុស (ស្រទន់)":
        st['v'] = "km-KH-PisethNeural"
        bot.send_message(cid, "✅ សំឡេងប្រុសរួចរាល់", reply_markup=get_kb(cid))
    elif "🌐 បកប្រែ" in t:
        st['tr'] = not st['tr']
        ...
        if st.get('mode') == 'ai':
            # មុខងារ AI ឆ្លើយ
            response = model.generate_content(f"Answer in Khmer: {t}")
            t = response.text
            st['mode'] = None
        process_voice(m, t)
